...
# Created on 2021/01/20 20:45:33
# ###########################################################################
...
grep "# ARGS" $0 | grep -v grep | cut -d" " -f3- | sed -e s%PYTHON_PATH_MARKER%"$(python -c "import sys;print('.' + ':'.join(sys.path))")"%g | sed -e s#PROJECTROOT0_MARKER#"$PROJECTROOT0"#g   > $ARGFILE
...
# ###########################################################################
echo Starting scan
$SOURCEANALYZER $MEMORY $LAUNCHERSWITCHES -b $BUILDID $SCANSWITCHES -scan -logfile scan.log -f $FPR
if [ $? -ne 0 ] ; then
echo sourceanalyzer failed, exiting
exit
fi
